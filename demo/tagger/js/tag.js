...
function updateTags() {
	...
	for (i in tags){
		...
		if (!(tag.includes("hvg") || tag.includes("ner__") || tag.includes("geography__") || tag.includes("geo__") || tag.includes("person__") || tag.includes("pers__") || tag.includes("organization__") || tag.includes("org__"))) {
			if(num >= aktProb) {
				predikalt.append(
					'<li>' +
...
function test6() {
	var szoveg = 'Ha azt mondjuk Szex és New York, az izgalmas csavarok mellett tuti, hogy a gyönyörű ruhák és menő szettek is eszedbe jutnak. Bár a sorozat már lassan jó sok éve véget ért, tudjuk, hogy a divat ismétli önmagát, így nem is csoda, hogy Carrie (Sarah Jessica Parker) néhány ikonikus darabja és összeállítása időről időre visszatér a köztudatba, és persze sokak ruhatárába. Most összeszedtünk nektek 7 ötletet, amit ti is a saját stílusotokhoz tudtok igazítani, és nagy valószínűséggel a darabok nagyrésze megtalálható a szekrényetekben. Biztosan mindenki megnézi magának ezt a gyönyörű szoknyát, mely tele van különleges részletekkel. A rajta lévő rózsák és a rengeteg réteg miatt sokáig a gardrób ékes dísze lehet. A hétköznapokban persze ennél visszafogottabb darabot válassz, de a siker garantált. Manapság nagyon menő a különböző minták és anyagok keverése, Carrie pedig tökéletesen ötvözi a bundát a krokodilmintával ebben az outfitben. Ha te is mixelnél, próbáld ki a bőrt a csipkével, vagy a bőrt bundával- természetesen kizárólag műbőrrel és műszőrmével. A retro és a vintage már jó pár éve újra felütötte fejét a divatvilágban, így érdemes körülnézni anyukád vagy nagymamád szekrényében, hátha van valami, amit tudnál hasznosítani. Carrie ebben az esetben egy 1920-as ihletésű ruhát vett fel, melynek klasszikus stílusa azonban időtlenné teszi a darabot. Manapság rengetegen a minimalista öltözködés hívei, így nagy valószínűséggel a te szekrényedben is lapul legalább egy egyszerű ruha. Ha azonban mégis vágynál egy kis extrára, válassz valamilyen élénk színt, mint ahogy Carrie tette a lilával, vagy kísérletezz az anyagokkal. A különböző stílusok keverése nagyon népszerűvé vált az utóbbi időben, így nem is csoda, hogy Carrie sportos-elegáns összeállítása is felkerült a listára. Legtöbbször a nőies ruhadarabokat párosítjuk egy-egy sportos darabbal, például egy klasszikus szoknyás szettet simán fel tudsz dobni egy papuccsal, vagy akár sportcipővel is.';
	upload(szoveg);
}
